...
namespace Cipher.Views {
    ...
    public class BeaufortCipherView : Gtk.Grid {
        ...
        public BeaufortCipherView () {
            var beaufort = new Cipher.Ciphers.Beaufort ();

            encipher_button.clicked.connect (() => {
                ciphertext_textview.buffer.text = beaufort.encrypt (plaintext_textview.buffer.text);
            });

            decipher_button.clicked.connect (() => {
                plaintext_textview.buffer.text = beaufort.decrypt (ciphertext_textview.buffer.text);
            });
        }
